...
## 官方 CDN 构建归档（2026-09-19 全线探明）

`https://dldir1v6.qq.com/weixin/Universal/Mac/xWeChatMac_universal_<点分版本>_<构建号>.dmg`
按构建号永久归档（非 XZ、无速率限制）。覆盖（HEAD 实测）：
- **4.1.15 全家族**（4.1.15.10-.20 = 270090-270100，除从未发布的 270092）
- **4.1.13 的 .5-.11 与 .50-.63 段**（= 269573-269579 与 269618-269631）
- 缺口：4.1.13.1-.4、.12-.49（含 269602=.34）、4.1.12 及更老全线——
  这些构建的官方原版字节无回填源（zsbai XZ 损坏频发，仅剩希望）
- 线性映射：4.1.15.N ↔ 270080+N、4.1.13.N ↔ 269568+N（20+ 直链实证）
...
269630:512c7f9 → 269631:512ca09 → 270090:5374e69 → … → 270100:537dfb9
（连续段内单调爬升；跨 .11→.34 缺口有一次代码重排，非单调）。

keeptip store（parse+偏移处 E8 call + `488983C8010000`，asm 家族恒定，
call rel32 按时代分五变体）：.5-.8=`E81E85E9FF`、.9-.11=`E8DE67E9FF`、
.50-.54=`E8BE67E9FF`、.56-.63=`E81E66E9FF`、4.1.15=`E83E4BE9FF`——
expected 必须按实读字节入库（㉚ 轮 18 构建全部如此）。
