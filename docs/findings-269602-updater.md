...
## v7（2026-09-17 深夜）：代码洞假 id

v6 失败后的决定性再推断：当前(v6)二进制中活路径对 +0x1C8 已零读取（穷举扫描
6 种寄存器编码×8 个区间），消息仍消失 → v1 的有效成分只是"标记引用 id 被改坏"
（JOIN 失配），与哪个执行器无关；执行器砍不完因为隐藏/删除是**数据驱动**。

**v7 补丁**（两处）：
- `0x68782c4`（函数间隙 ret 后 nop 填充,8B）写入 stub `mov rax,1; ret`
- `0x50a5bad` 的 call 重指向该洞：E8BE26E9FF → E812DC7D01（store 不动）
→ 标记引用 id = 1：非零（区别于 v1 的 0，群聊校验有机会通过）+ JOIN 永不失配
（消息保留）。登录重放与全部执行器逻辑不变。

## v7 终局（崩溃=决定性数据）

非零假 id(=1)导致微信闪退：下游按 id 查找无空值防护，0 会短路跳过而 1 会真的
执行。**"改坏 id 保消息"只剩 0 一个安全值**——即 v1。结合 fzlzjerry 文档核对：
其 arm64 tip 模式正是同款（str xzr 写零 newmsgid，"保留原消息仍显示撤回提示"），
