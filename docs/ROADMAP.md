...
53 轮静态站位图的实弹校准——一处分支误读被纠正。

1. **时间线解码（关键方法论收获）**：撤回前 +175~194s 有 8 轮
   dispatch/post 全站命中——调用者是 0x35217ff（16 共用调用者之一）而非
   apply，属**无关基线同步流量**（其向量元素未过 pred 谓词，action 零
   命中）；撤回于 +201s 到达（parse 对 297/155），此后 **零** dispatch/
   action/flagw。结论：0x34f4440 批次处理器是通用设施（多子系统共用），
   **apply 的尾部投递分支（0x3510b9b→0x351f4e0）在真实撤回流中未走**——
   53 轮把该分支当主路径是静态分支误读，实弹纠正。
2. **模型修正**：apply（0x35103d0，若 hit 路径到达）内部有多条退出路径，
   真实流走的退出不含尾部投递；删除与 tip 插入在 apply 更早的子调用——
   头号嫌疑 = **0x369c620（apply 专属被调，全镜像唯一调用者，0x18e0B，
   SSO 串参数+重度解密）**，次之为 0x5310620/0x334fc50/0x2c900f0。
3. **drive35 方案（下一会话，需一次撤回）**：NATIVE 态 bp apply 内部
   调用位点（0x3510425→0x369c620 返回、0x351069c、0x3510b7a、0x3510b9b、
   0x3510baa）+ 0x394c4e9（apply 入口确认）——一次撤回即知哪些子调用
   真实发生，删除/tip 各归其位后 M-R4 v2 hook 落址。
4. **状态**：drive34 数据 var/wxarm/d34.log、d34_parse_1/2.xml；
   日常态恢复（keeptip patched / protected / verify OK）。

## 53·drive33 续成轮（2026-09-25 深夜：撤回执行机制精化——apply 末端是「消息批次再入库」管线，Message 对象 stride 0x278 实锤）

任务：drive33 续（0x34f4440 分派器解剖）。撤回执行机制从「黑盒 apply」
精化为可定位的管线图。

1. **0x34f4440 = 消息批次处理器**（0x160B，apply 末端经 0x351f4e0 到达）：
   - 迭代的向量元素 = **0x278 步长的 Message 对象**（与 ㉘ 社区实证的
     Message 尺寸 632B 完全一致——apply 把操作对象组织成消息批次）；
   - Phase 1 逐消息：pred2=`cmp [msg+0x10],0x18`（type==24）、pred1=
     type==0x13 或 [msg+0x220] 非空检查 → 命中者 **call 0x530e280(msg,1)**；
   - Phase 4 逆序 vtable 调用：`call [obj+0](obj)`（0x34f4520 位点）；
   - Phase 5 四个后处理：0x34f45a0（大函数 0x160B+）/0x34f5560/
     0x34f5660/0x34f5840/0x34f5940。
2. **[身份实锤] 0x530e280 = A 路漏斗函数本体**：其内部 0x530e2b8 的
   vtable 分派后、0x530e2bb（drive29/30 bt 里的漏斗返回址）就在本函数——
   即「A 路 parse 调用者」= 这个 per-message 入库/再解析动作。撤回链的
   端到端图景闭合：**apply 把消息批次逐个重新喂进消息管线（vtable 分派
   [msg+0x220] → parse）**，删除与 tip 插入均在这条再入库流的下游。
3. **M-R4 含义更新**：干预点候选收窄到两处——(a) Phase 1 的谓词/动作对
   （0x530e280 调用位点 0x34f4480：hook 此处选择性跳过目标消息的再入库
   = 废删除）；(b) Phase 4 vtable 调用（0x34f4520）与 Phase 5 后处理
   （tip 插入嫌疑最大）。**drive34 实弹定位**：NATIVE 态 bp
   0x34f4440/0x34f4480/0x34f4520/0x34f45a0/0x34f5560-5940/0x3510baf
   （+0x278 状态位写），一次群聊撤回抓各站 bt——tip 插入与删除各归其位
   后即可设计 v2 hook（预期形态：0x34f4480 位点的 msg 过滤，与 parse
   hook 同款地址表机制）。
4. **工具**：var/wxarm/d33_partial.log（apply 全景 + 分派器 + 任务投递
   反汇编存档）；drive31_static.py 复用。
5. **状态**：origin 同步至 a0676fb；日常防护态在位。

## 52·drive33 起步轮（2026-09-25 深夜：apply 内部地图 v1——任务投递/状态位写入现形，拆分待续）

任务：drive33 静态（apply=0x35103d0 内部拆分删除 vs tip 插入）。本轮完成
地图 v1，完整拆分留下一会话（规模：0x970B 函数 + 观察者分派链）。

1. **apply 结构（0x35103d0..0x3510d40）**：日志重度（revoke_manager.cc
   行号 0xbfe/0xbf9 等）；中段 `call 0x5310620(out, r13串)`（r13 来自
   0x336d5c0 串转换）；尾段三连：
   `call 0x334fc50(out, r13) → call 0x351f4e0(ctx, wxid, out) →
   call 0x2c900f0(r12, out) → mov byte [r12+0x278], 1`（★ 消息对象
   +0x278 状态位写 1——与 ㉘ +0x118 状态机并行的另一状态面）。
2. **0x351f4e0 = 观察者分派器**（16 handler 共用）：先 0x34fa6f0 →
   `call 0x34f4440(ctx, wxid, out)` 分派 → 遍历 out 内**向量**
   （stride 除法魔数 0x9b8b577e613716af）逐元素处理——tip 插入/删除的
   最终执行体在向量元素的分派终点（0x34f4440 内部或元素 handler）。
3. **下一会话 drive33 续**：解剖 0x34f4440 分派 + 向量元素 handler →
   分离「删除原消息」与「插入 tip」两个动作点 → M-R4 v2 hook 设计
   （目标：hit 路径内仅废删除，保 tip）。工件 var/wxarm/d33_partial.log。
4. **状态**：origin 同步至 3fb2e9b（51 轮）；日常防护态在位。
