...
`test al,al` 条目地址，位于 parse 函数体内）。派生链（270099 拓扑，drive22
实证：wrapper 是 parse 的唯一调用者，call 位于 wrapper+0x1C3）：

  guard(parse 体内) --LC_FUNCTION_STARTS--> parse 入口
  parse 入口 <--E8 唯一调用者-- wrapper 内部
  wrapper --LC_FUNCTION_STARTS--> wrapper 入口 → 12B 序言 + LC_UUID → hooks 行
...
def main():
    ...
    off = machutil.va2off(d, wrapper_entry)
    prologue = d[off:off + 12]
    if prologue != EXPECTED_PROLOGUE:
        raise SystemExit(
            f"wrapper {wrapper_entry:#x} 序言 {prologue.hex().upper()} != 纯栈操作门"
    ...
    row = {
        ...
        "uuid": uuid,
        "arch": "x86_64",
        "hook_off": format(wrapper_entry, "#x"),
        "msg_arg": 1,
        "xml_sso_off": 0x130,
