...
# drive21：M-R2 tip 物化验证 + 修正版 SSO 回溯（270099 x64）。
...
def drive21(debugger, command, result, internal_dict):
    ...
    for m in target.modules:
        ...
        if err.Success() and gt.hex() == '554889e553504889fb':
            ...
            break
    if base is None:
        print('DRIVE21: base 不明', flush=True)
        return
    print(f'DRIVE21: base={base:#x}', flush=True)
    ...
    print(f'DRIVE21: heap regions={len(regions)} total={tot/(1<<20):.0f}MB', flush=True)
    ...
    for begin, sz in regions:
        ...
        while off < sz:
            ...
            while True:
                ...
                pos = i + 1
            off += n
    print(f'DRIVE21: needle hits={len(hits)}', flush=True)
    err2 = lldb.SBError()
    for h in hits[:12]:
        ctx = proc.ReadMemory(max(0, h - 40), 100, err2)
        print(f'  needle@{h:#x} ctx={ctx[:100]!r}' if err2.Success() else f'  needle@{h:#x}', flush=True)

    by_region = {}
    for h in hits:
        for begin, sz in regions:
            if begin <= h < begin + sz:
                by_region.setdefault(begin, []).append(h)
                break
    picked = []
    for begin in sorted(by_region):
        picked.append(by_region[begin][0])
        if len(picked) >= WP_CAP:
            break
    print(f'DRIVE21: picked={ [hex(x) for x in picked] }', flush=True)
    if not picked:
        print('DRIVE21: 无命中 — detach', flush=True)
    ...
    for wa in picked:
        ...
        if ro.Succeeded():
            armed += 1
            print(f'DRIVE21: read-WP#{armed} @0x{wa:x}', flush=True)
    print(f'DRIVE21: ARMED ({armed}) — 等待读取', flush=True)
    ...
    while n < WP_HITS_CAP:
        ...
        if state in (lldb.eStateExited, lldb.eStateInvalid, lldb.eStateDetached):
            print(f'DRIVE21: gone ({n})', flush=True)
            return
        ...
        for t in proc:
            ...
            break
    print('DRIVE21: 捕获完成 — detach', flush=True)
    ci.HandleCommand('watchpoint delete', ro)
    ci.HandleCommand('process detach', ro)


def __lldb_init_module(debugger, internal_dict):
    debugger.HandleCommand('command script add -f drive21.drive21 drive21')
