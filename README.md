...
- **行为验证**——`verify` 拉补丁函数出进程直接调用，机器证明有效（x64 探针=补丁后的 isRevokemsg 本体；arm64 探针=喂值给 cbz 分支的独立谓词，家族完整性+输入路径自检，补丁效果由 strict verify 字节级证明承担）
...
- verify 的行为验证在 AMFI 活跃的机器上不可用（RWX 映射被禁，与 SIP 开关独立）；verify 会先打环境提示（`doctor` 有精确解法）；CI 上自动跳过。arm64 行为验证的 worker 执行路径待 ARM 实机验收（解码/布局逻辑已有跨架构单测锁死）
